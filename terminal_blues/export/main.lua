dofile('elements/errors.lua')

local sprite=app.activeSprite

if sprite==nil then
    app.alert(errors.active_sprite_nil)
    return
end
...
if dat.ok then
    ...
    if dat.custom_export then
        if dat.custom_dir=='' then
            app.alert(errors.empty_custom_dir)
            return
        end
        -- Add a slash at the end of the dir in case the user hasn't
        if string.byte(s, #s)~=string.byte('/') then
            s=s..'/'
        end
    ...
    if dat.repo_blues and pal_check(repo_dir, 'blues', dat.ver_blues) then
        ...
        export(sprite, repo_dir, false, true, true)
    end
